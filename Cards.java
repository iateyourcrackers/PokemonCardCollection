 ...
 * 
 * It assigns a new unique ID for each new Card object added to the collection, and provides methods to: 
 * add new cards to the collection with and without an image path; 
 * search for a Card object in the collection by using the card's name;
 * and retrieve Card objects from within the collection by its ID.
 * 
 * @author (HFJ)
 * @version (20/5/25)
 */
public class Cards
{
    ...
    /**
     * Search for a card in the collection by using its name.
    ...
    public boolean findCard(String name) {
        ...
        for (int cardId: collection.keySet()) {
            ...
            if (collection.get(cardId).getName().equals(name)) {
                this.currCard = this.collection.get(cardId); // save the card match found in currCard variable
                return true;
            }
        }
    ...
    /**
     * Add a new card (without an image path) to the collection.
    ...
    /**
     * Add a new Card (with an image path) to the collection.
     * @param name the card's name
     * @param value the card's monetary value
    ...
    /**
     * @return the most recent Card object found
     */
    ...
    /**
     * @return a Card object in the collection by its unique ID
     */
    ...
    /**
     * @return the highest, most recently assigned ID (e.g. number of cards in the collection)
     */
